...
pathways<-lapply(1:5, function(i) pathways[[i]])
ls()
@
...
pathways<-pathways("hsapiens", "kegg")
pathways<-lapply(1:10, function(i) pathways[[i]])
@ 
...
<< fig=true, width=8, height=7, eval=FALSE>>=
library(gageData)
data(hnrnp.cnts)

group<-c(rep("sample",4), rep("control",4))
hnrnp.cnts<-hnrnp.cnts[rowSums(hnrnp.cnts)>0,]
cli<-Clipper(hnrnp.cnts, group, pathways[1:2], type="RNASeq", testCliques=TRUE)
