...
The {\tt pathway} class is deprecated in the version 1.13.0 of \texttt{graphite} package and substitued with \texttt{PathwayList} and \texttt{Pathways}. However, the implementation of the topology-based methods in ToPASeq still requires pathways as \texttt{pathway}-objects. Therefore, a special method that converts \texttt{Pathways} into \texttt{pathway} was created. 
<<>>=
library(ToPASeq)
